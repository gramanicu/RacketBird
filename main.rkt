...
(define-struct Bird (pos vel) #:transparent)
(define-struct Variables (gravity momentum scroll-speed score) #:transparent)

(define-struct State (bird vars pipes) #:transparent)
...
(define-struct Pipe(pos) #:transparent)
...
(define (get-initial-state)
  (State
    (Bird (make-posn bird-x bird-initial-y) 0)
    (Variables initial-gravity initial-momentum initial-scroll-speed 0)
    (list (Pipe (make-posn scene-width (+ added-number (random random-threshold)))))
   )
 )
...
(define(get-pipe-x pipe)
  (posn-x (Pipe-pos pipe)))

(define(get-pipe-y pipe)
  (posn-y (Pipe-pos pipe)))
